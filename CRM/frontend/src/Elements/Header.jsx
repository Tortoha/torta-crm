      ...
      <div className="hdr-prod-panel" onMouseLeave={() => setItemHov(null)}>
        ...
        {items.map(({ slug, Icon }) => (
          <button key={slug} type="button"
            ...
            onClick={() => { setOpenState(false); navigate(lp(`/${slug}`)); }}>
...
function HeaderLandingMobile() {
  ...
  const lp = useLocalePath();
  const go = (to) => { setOpen(false); navigate(lp(to)); };
  ...
  return (
    <header className={`crm-header${landingWide ? ' crm-header--landing' : ''}`}>
      <div className="hdr-left">
        ...
        <button className="hdr-brand" onClick={() => navigate(landing ? lp('/') : (user ? '/dashboard' : '/'))} type="button" aria-label="Torta CRM">
